...
class GoodsadminController extends AdminbaseController {
...
//		$this->term_relationships_model = D("Portal/TermRelationships");
	...
	public function add_ali(){
		
		$this->display();
	}
	// 文章添加提交
	public function add_ali_post(){
            
            
            $pages=$_GET['pages']?intval($_GET['pages']):1;
            
//            $toPage=$_GET['toPage']?intval($_GET['toPage']):1;
            $catIds=$_GET['catIds']?intval($_GET['catIds']):1;
            $level=$_GET['level']?intval($_GET['level']):1;
            $perPageSize=$_GET['perPageSize']?intval($_GET['perPageSize']):40;
            $dpyhq=$_GET['dpyhq']?intval($_GET['dpyhq']):1;
           if($dpyhq==1){
               $shopTag="dpyhq";
           }
                ...
                for($i=1; $i<=$pages; $i++){
                        ...
                        $content = "";  
                        if($_GET['channel']=="qqhd"){
                        $handle = fopen("http://pub.alimama.com/items/channel/qqhd.json?toPage=$i&catIds=$catIds&level=$level&perPageSize=$perPageSize&shopTag=$shopTag","rb");  
                        }
                        elseif($_GET['channel']=="tehui"){
                           
                        $handle = fopen("http://pub.alimama.com/items/channel/tehui.json?channel=tehui&toPage=$i&catIds=$catIds&level=$level&perPageSize=$perPageSize&shopTag=$shopTag","rb");  
                        ...
                        fclose($handle);
                        $goods_res=json_decode($content);
//                        echo '<pre>'; print_r($goods_res->data->pageList);die();
                         foreach($goods_res->data->pageList as $v){
                             
                            
                             $data_goods=array(
//                                 'eventCreatorId'=>$v->eventCreatorId,
//                                 'rootCatId'=>$v->rootCatId,
//                                 'leafCatId'=>$v->leafCatId,
//                                 'debugInfo'=>$v->debugInfo,
//                                 'rootCatScore'=>$v->rootCatScore,
                                 'sellerId'=>$v->sellerId,
                                 'userType'=>$v->userType,
                                 ...
                                 'dayLeft'=>$v->dayLeft,
//                                 'tk3rdRate'=>htmlspecialchars($v->tk3rdRate,ENT_QUOTES),
                                 'tk3rdRate'=>$v->tk3rdRate,
                                 'zkPrice'=>$v->zkPrice,
                                 'tkRate'=>$v->tkRate,
                                 ...
                                 'totalNum'=>$v->totalNum,
                                 'totalFee'=>$v->totalFee,
//                                 'sameItemPid'=>$v->sameItemPid,
//                                 'hasSame'=>$v->hasSame,
//                                 'hasRecommended'=>$v->hasRecommended,
                                 'couponTotalCount'=>$v->couponTotalCount,
                                 'couponLeftCount'=>$v->couponLeftCount,
                                 'auctionUrl'=>$v->auctionUrl,
                                 'couponLink'=>$v->couponLink,
                                 'couponLinkTaoToken'=>$v->couponLinkTaoToken,
                                 'includeDxjh'=>$v->includeDxjh,
                                 'auctionTag'=>$v->auctionTag,
                                 'couponAmount'=>$v->couponAmount,
                                 'eventRate'=>$v->eventRate,
                                 'couponShortLink'=>$v->couponShortLink,
                                 'couponInfo'=>$v->couponInfo,
                                 'couponStartFee'=>$v->couponStartFee,
                                 'couponEffectiveStartTime'=>$v->couponEffectiveStartTime?strtotime($v->couponEffectiveStartTime):0,
                                 'couponEffectiveEndTime'=>$v->couponEffectiveEndTime?strtotime($v->couponEffectiveStartTime):0,
//                                 'hasUmpBonus'=>$v->hasUmpBonus,
//                                 'isBizActivity'=>$v->isBizActivity,
//                                 'umpBonus'=>$v->umpBonus,
//                                 'rootCategoryName'=>$v->rootCategoryName,
                                 'couponOriLink'=>$v->couponOriLink,
//                                 'userTypeName'=>$v->userTypeName,
                             ...
                             if($v->tkSpecialCampaignIdRateMap){
                                 foreach($v->tkSpecialCampaignIdRateMap as $k1=>$v1){
                                     $rate_arr[$k1]=$v1;
                                 }
                             }
                            $rate_arr['tkRate']=$v->tkRate;
                            $rate_arr['eventRate']=$v->eventRate;
                             
                            $keys=array_search(max($rate_arr),$rate_arr); 
                            
                            if($keys=="eventRate"){
                                $data_goods['out_type']=3;
                            }elseif($keys=="tkRate"){
                                $data_goods['out_type']=1;
                            }else{
                                $data_goods['out_type']=2;
                            }
                          
                            $data_goods['out_rate']=$rate_arr[$keys];

                            $data_goods['out_CommFee']=number_format($v->zkPrice*$rate_arr[$keys]/100, 2, '.', '');
                             
//                             echo '<pre>';  print_r($data_goods);die();
                              
                                                if(!$this->goods_model->where(array('auctionId'=>$v->auctionId))->select()){
                                                      // add
                                                      $data_goods['add_time']=time();
                                                      $this->goods_model->add($data_goods);
                                                }
...
//	}
	...
	public function delete(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->goods_model->where(array('id'=>$id))->save(array('post_status'=>3)) !==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if(isset($_POST['ids'])){
			...
			if ($this->goods_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>3))!==false) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	public function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>1)) !== false ) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('post_status'=>0)) !== false) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
	...
	public function top(){
		if(isset($_POST['ids']) && $_GET["top"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>1))!==false) {
				$this->success("置顶成功！");
			} else {
				$this->error("置顶失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["untop"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('istop'=>0))!==false) {
				$this->success("取消置顶成功！");
			} else {
				$this->error("取消置顶失败！");
			}
	...
	public function recommend(){
		if(isset($_POST['ids']) && $_GET["recommend"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>1))!==false) {
				$this->success("推荐成功！");
			} else {
				$this->error("推荐失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["unrecommend"]){
			...
			if ( $this->goods_model->where(array('id'=>array('in',$ids)))->save(array('recommended'=>0))!==false) {
				$this->success("取消推荐成功！");
			} else {
				$this->error("取消推荐失败！");
			}
	...
	public function copy(){
	    if(IS_POST){
	        if(isset($_GET['ids']) && isset($_POST['term_id'])){
	            ...
	            foreach ($ids as $id){
	                $find_post=$this->goods_model->field('post_keywords,post_source,post_content,post_title,post_excerpt,smeta')->where(array('id'=>$id))->find();
	                if($find_post){
	                    $find_post['post_author']=$uid;
	                    $find_post['post_date']=date('Y-m-d H:i:s');
	                    $find_post['post_modified']=date('Y-m-d H:i:s');
	                    $post_id=$this->goods_model->add($find_post);
	                    if($post_id>0){
	...
	public function clean(){
		if(isset($_POST['ids'])){
			$ids = I('post.ids/a');
			$ids = array_map('intval', $ids);
			$status=$this->goods_model->where(array("id"=>array('in',$ids),'post_status'=>3))->delete();
			...
			if(isset($_GET['id'])){
				$id = I("get.id",0,'intval');
				$status=$this->goods_model->where(array("id"=>$id,'post_status'=>3))->delete();
	...
	public function restore(){
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			if ($this->goods_model->where(array("id"=>$id,'post_status'=>3))->save(array("post_status"=>"1"))) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
